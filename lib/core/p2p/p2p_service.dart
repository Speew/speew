...
import '../mesh/priority_queue_mesh_dispatcher.dart';
...
class P2PService extends ChangeNotifier {
  ...
  late final PriorityQueueMeshDispatcher _dispatcher;

  Future<void> initialize() async {
    try {
      _dispatcher = PriorityQueueMeshDispatcher(_reputationCore, this);
    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'coin_transaction':
          ...
          break;
        case 'audio':
          priority = MessagePriority.REAL_TIME;
          break;
        default:
          priority = MessagePriority.REAL_TIME;
      }
      ...
      logger.info('Mensagem enfileirada para ${peer.displayName}: ${message.type} (QoS: $priority)', tag: 'P2P');
      
      if (message.type == 'file_transfer_request') {
        final filePath = message.payload['filePath'] as String;
        final fileId = message.payload['fileId'] as String;
        await _sendFileInChunks(peerId, fileId, filePath);
      }
      
      return true;
    } catch (e) {
      logger.info('Erro ao enfileirar mensagem: $e', tag: 'P2P');
      return false;
    }
  }

  Future<bool> sendData(String peerId, String data, {required MessagePriority priority, Map<String, dynamic>? metadata}) async {
    ...
    if (priority == MessagePriority.BULK) {
      ...
      finalData = 'ZSTD_COMPRESSED(${data.substring(0, (data.length * 0.5).toInt())})';
      isCompressed = true;
      logger.debug('Compressão Zstd aplicada. Tamanho original: $originalSize, Final: ${finalData.length}', tag: 'P2P');
    } else if (priority == MessagePriority.REAL_TIME || priority == MessagePriority.CRITICAL) {
      logger.debug('Compressão ignorada para tráfego de alta prioridade: $priority', tag: 'P2P');
    }
    
    logger.debug('Enviando dados brutos para $peerId. Tipo: ${metadata?['fileId'] != null ? 'Chunk' : 'Mensagem'} (Comp: $isCompressed)', tag: 'P2P');
    
    if (DateTime.now().millisecond % 100 == 0) {
      logger.error('Simulação de falha de rede para $peerId', tag: 'P2P');
      throw P2PException.networkError('Falha de rede simulada');
    }
